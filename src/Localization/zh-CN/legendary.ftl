...
legendary-no-linked-account = 未找到已链接的 { $companyAccount } 账户！通过浏览器链接您的账户并重试。
legendary-all-activated-ubisoft = 所有游戏已在您的 { $companyAccount } 账户上激活。
legendary-continue-activation = 您确定要在您的 { $companyAccount } 账户上激活游戏吗？
legendary-games-activate-success = 所有游戏已在您的 { $companyAccount } 账户上成功激活。
legendary-activate-games = 激活游戏
legendary-games-activate-failure = 在您的 { $companyAccount } 账户上激活游戏失败。原因：{ $reason }。
legendary-no-available-ea-games = 没有可用的 EA 游戏。
legendary-activate-next-confirm = 您要激活下一个游戏吗？
...
legendary-alternative-auth-instructions-second = 2. 导航到下面的输入框中显示的身份验证代码 URL。
legendary-alternative-auth-instructions-third = 3. 将您的 “authorizationCode” 值粘贴到下面的输入框中，然后单击 “authenticate” 按钮。
legendary-disable = 禁用
legendary-enable = 启用
