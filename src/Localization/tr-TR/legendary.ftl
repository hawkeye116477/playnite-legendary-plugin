...
legendary-no-linked-account = Bağlı bir { $companyAccount } hesabı bulunamadı! Hesabınızı tarayıcınız aracılığıyla bağlayın ve tekrar deneyin.
legendary-all-activated-ubisoft = { $companyAccount } hesabınızda tüm oyunlar zaten etkinleştirildi.
legendary-continue-activation = { $companyAccount } hesabınızda oyunları etkinleştirmek istediğinizden emin misiniz?
legendary-games-activate-success = { $companyAccount } hesabınızdaki tüm oyunlar başarıyla etkinleştirildi.
legendary-activate-games = Oyunları etkinleştir
legendary-games-activate-failure = { $companyAccount } hesabınızdaki oyunlar etkinleştirilemedi. Sebep: { $reason }.
legendary-no-available-ea-games = Kullanılabilir EA Oyunu yok.
legendary-activate-next-confirm = Bir sonraki oyunu etkinleştirmek istiyor musunuz?
...
legendary-alternative-auth-instructions-second = 2. Aşağıdaki giriş kutusunda gösterilen kimlik doğrulama kodu adresine gidin.
legendary-alternative-auth-instructions-third = 3. Aşağıdaki giriş kutusuna "yetki kodu" değerinizi yapıştırın ve "doğrula" düğmesine tıklayın.
legendary-disable = Etkisizleştir
legendary-enable = Etkinleştir
