...
legendary-third-party-launcher-import-warn = Для таких игр не требуется Legendary, и не все из них можно установить через него. Рекомендуется перейти в раздел «Облако и другие сервисы» => { $thirdPartyLauncherName } => Активировать игры, а затем установить их напрямую через { $thirdPartyLauncherName }.
...
legendary-disable = Отключить
legendary-enable = Включить
legendary-game-import-failure = Не удалось импортировать игру. Причина: { $reason }.
