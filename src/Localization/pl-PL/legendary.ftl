legendary-launch-offline = Uruchamiaj gry w trybie offline, jeśli to możliwe
legendary-enable-offline-mode = Włącz tryb offline
legendary-offline-mode = Tryb offline
...
legendary-no-linked-account = Nie znaleziono powiązanego konta { $companyAccount }! Połącz swoje konto za pomocą przeglądarki i spróbuj ponownie.
legendary-all-activated-ubisoft = Wszystkie gry zostały już aktywowane na Twoim koncie { $companyAccount }.
legendary-continue-activation = Czy na pewno chcesz aktywować gry na swoim koncie { $companyAccount }?
legendary-games-activate-success = Wszystkie gry zostały pomyślnie aktywowane na Twoim koncie { $companyAccount }.
legendary-activate-games = Aktywuj gry
legendary-games-activate-failure = Nie udało się aktywować gier na Twoim koncie { $companyAccount }. Powód: { $reason }.
legendary-no-available-ea-games = Brak dostępnych gier EA.
legendary-activate-next-confirm = Czy chcesz aktywować kolejną grę?
...
legendary-required-install-via-third-party-launcher-error =
    { $count ->
       ...
       *[other] Następujące gry wymagają bezpośredniej instalacji przez { $thirdPartyLauncherName }: { $gameTitle }.
    }
...
legendary-alternative-auth-instructions-first = 1. Zaloguj się do Epic Games Store w swojej przeglądarce internetowej.
legendary-alternative-auth-instructions-second = 2. Przejdź do adresu URL kodu uwierzytelniającego widocznego w poniższym polu.
legendary-alternative-auth-instructions-third = 3. Wklej wartość „authorizationCode” w poniższym polu i kliknij przycisk „uwierzytelnij”.
legendary-disable = Wyłącz
legendary-enable = Włącz
